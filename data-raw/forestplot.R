...
meta_fil %>%
  mutate(color = ifelse(apply(
    cbind(meta_fil$lowerte, meta_fil$upperte),
    1, function(x) findInterval(1, x)
  ) == 1,
  "contains", "does not contain"
  )) %>%
  ggplot(aes(
    y = studlab, x = te, xmin = lowerte,
    xmax = upperte, col = color
  )) +
  geom_point() +
...
meta_fil <- meta %>%
  ...
  distinct() %>%
  mutate(
    dist = ifelse(is.na(evente), "Student-T", "Binomial"),
...
CI_s <- meta_fil$study_class %>%
  ...
  purrr::map_dfr(function(x) {
    ...
    # Adding fixed lower, upper and point estimation
    lower_fixed <- meta_model %>% purrr::pluck("lower.fixed")
    upper_fixed <- meta_model %>% purrr::pluck("upper.fixed")
    point_estim_fixed <- meta_model %>% purrr::pluck("TE.fixed")

    list(
      study_class = x, "CIlower" = lower, "CIupper" = upper, "mean" = point_estim,
      "CIlower_fixed" = lower_fixed, "CIupper_fixed" = upper_fixed, "mean_fixed" = point_estim_fixed
    )
  })

# analysing model
used_data <- meta_fil |> filter(study_class == 1)
...
pooled <- meta_fil %>%
  mutate(
    sume = meane * ne, sumc = meanc * nc,
    SQe = sde^2 * (ne - 1), SQc = sdc^2 * (nc - 1)
  ) %>%
  group_by(dist, study_class) %>%
  summarise(
    ne = sum(ne), evente = sum(evente),
...
# duplicating the pooled rows
meta_fil <- bind_rows(
  meta_fil,
  meta_fil |> filter(studlab == "Pooled") |> mutate(studlab = paste0(studlab, " (fixed effects)"))
) |>
  mutate(studlab = ifelse(studlab == "Pooled",
    "Pooled (REM)", studlab
  ))
...
meta_fil <- meta_fil %>%
  mutate(
    CIlower = ifelse(studlab == "Pooled (REM)" & study_class == 1,
      CI_s$CIlower[1],
      ifelse(studlab == "Pooled (REM)" & study_class == 2,
        CI_s$CIlower[2],
        ifelse(studlab == "Pooled (fixed effects)" & study_class == 1,
          CI_s$CIlower_fixed[1],
          ifelse(studlab == "Pooled (fixed effects)" & study_class == 2,
            CI_s$CIlower_fixed[2], CIlower
          )
        )
      )
    ),
    CIupper = ifelse(studlab == "Pooled (REM)" & study_class == 1,
      CI_s$CIupper[1],
      ifelse(studlab == "Pooled (REM)" & study_class == 2,
        CI_s$CIupper[2],
        ifelse(studlab == "Pooled (fixed effects)" & study_class == 1,
          CI_s$CIupper_fixed[1],
          ifelse(studlab == "Pooled (fixed effects)" & study_class == 2,
            CI_s$CIupper_fixed[2], CIupper
          )
        )
      )
    ),
    mean = ifelse(studlab == "Pooled (REM)" & study_class == 1,
      CI_s$mean[1],
      ifelse(studlab == "Pooled (REM)" & study_class == 2,
        CI_s$mean[2],
        ifelse(studlab == "Pooled (fixed effects)" & study_class == 1,
  ...
  select(
    studlab, study_class, ne, evente, meane, sde, nc, eventc, meanc, sdc,
...
g <- REACT::REACT_forestplot(CI_mat,
  NNT = NNT, study_names = meta_fil$studlab,
  point_estim = meta_fil$mean, prop_precision = TRUE
)
g$data$studlab
g$data$studlab <- relevel(g$data$studlab, "Pooled (REM)")
g$data$studlab <- relevel(g$data$studlab, "Pooled (fixed effects)")
...
# highlighting pooled
bold.labels <- ifelse(levels(g$data$studlab) %in% c(
  "Pooled",
  "Pooled (fixed)"
),
"bold", "plain"
...
meta_data %>%
  ...
  ggplot2::theme_bw() +
  ggplot2::labs(
    title = "NNT-based REACT Forestplot",
