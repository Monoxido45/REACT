...
ggplot(aes(x = x, y = y), data = points_data) +
  ...
  geom_abline(intercept = -delta, linetype = "dashed") +
  coord_cartesian(
    xlim = c(new_mu[1] - mu_dif - delta, new_mu[1] + mu_dif + delta),
...
p <- REACT::m_comparisons(
  alpha = 0.05, nrow = 1, ncol = 3,
  tol = delta, par = mu, f_matrix = var_cov,
  x_lims = c(50, 120), y_lims = c(50, 120)
)
...
plot_CI <- function(camcog_data, names_list = c("MCI", "AD", "CG"),
  ...
  for (i in 1:(length(names_list) - 1)) {
    for (j in (i + 1):length(names_list)) {
      ...
      group_1_obs_info <- new_camcog_data |>
        filter(Diagnostico == name_1) |>
        slice(1:2)

      # removing the two last entries from second group
      group_2_obs_info <- new_camcog_data |>
        filter(Diagnostico == name_2) |>
        slice(1:2)
  ...
  for (i in 1:(length(names_list) - 1)) {
    for (j in (i + 1):length(names_list)) {
      ...
      for (seed in seeds_list) {
        ...
        # removing the two first entries from first group
        group_1_obs <- camcog_data |>
          filter(Diagnostico == name_1) |>
          slice(1:2) |>
          pull(CAMCOG)

        # removing the two last entries from second group
        group_2_obs <- camcog_data |>
          filter(Diagnostico == name_2) |>
          slice_tail(n = 2) |>
          pull(CAMCOG)
...
m_comparisons_bayes_NGI <- function(theta, lambda, alpha_par, beta, stats, B, tol,
  ...
  # generating HPD data for prior
  df_p <- 2 * alpha_par
  hpd_prior_data <- generate_prior_HPD_data(
    theta, lambda, alpha_par, beta,
  ...
  for (i in 1:(length(names_list) - 1)) {
    for (j in (i + 1):length(names_list)) {
      ...
      data_used_prior <- data.frame(
        x = ch_points_prior |> pull(1),
      ...
      plot_list[[c]] <- ggplot2::ggplot(
        ggplot2::aes(x = x, y = y, fill = col, colour = col, linetype = type),
        data = all_data_used
      ) +
        ggplot2::geom_polygon(alpha = 0.4) +
        ggplot2::scale_color_manual(
...
prior_points <- DescTools::DrawEllipse(
  x = 80, y = 80,
